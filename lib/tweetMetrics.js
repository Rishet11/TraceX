...
async function fetchTweetMetrics(tweetId, timeoutMs = 4000) {
  ...
  return {
    ...
    bookmarks: Number(data?.stats?.bookmarks) || 0,
...
async function fetchTweetMetricsViaNitter(tweetId, timeoutMs = 5000) {
  ...
  return {
    ...
    views: Number(details?.stats?.views) || 0,
    bookmarks: Number(details?.stats?.bookmarks) || 0,
  ...
  const targets = results
    ...
    .filter((entry) => {
      ...
      const hasUsefulStats =
        (stats.likes || 0) > 0 || (stats.retweets || 0) > 0 || (stats.replies || 0) > 0;
      const missingViewsOrBookmarks = (stats.views || 0) <= 0 || (stats.bookmarks || 0) <= 0;
      const missingDate = !entry.result?.date || entry.result.date === 'Unknown';
      return !hasUsefulStats || missingDate || missingViewsOrBookmarks;
    })
    .slice(0, limit);
  ...
  for (let i = 0; i < targets.length; i += 1) {
    ...
    merged[target.index] = {
      ...
      stats: {
        ...
        views: (currentStats.views || 0) > 0 ? currentStats.views : metrics.views,
        bookmarks: (currentStats.bookmarks || 0) > 0 ? currentStats.bookmarks : metrics.bookmarks,
      },
    };
  }
