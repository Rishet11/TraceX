...
export async function getTweetDetailsById(tweetId, options = {}) {
  ...
  try {
    const fallback = await fetchSyndicationTweetById(tweetId, { timeoutMs });
    return fallback;
  } catch (syndicationError) {
    fallbackError = syndicationError;
    logSearchDebug(`Syndication fallback failed for tweet ${tweetId}: ${syndicationError.message}`);
  }

  try {
    const fallback = await fetchOEmbedTweetById(tweetId, { timeoutMs });
    return fallback;
  } catch (oembedError) {
    fallbackError = oembedError;
    logSearchDebug(`oEmbed fallback failed for tweet ${tweetId}: ${oembedError.message}`);
  }
