...
import { fetchJinaStatusTweetById, fetchOEmbedTweetById, fetchSyndicationTweetById } from './syndication.js';
import { fetchWithTimeout, UA_CURL } from './httpClient.js';

const NITTER_FETCH_OPTIONS = {
  headers: {
    'User-Agent': UA_CURL,
    Accept: '*/*',
  },
...
const URL_FALLBACK_SOURCES = ['syndication', 'oembed', 'jina_status'];
...
export async function searchNitter(query, options = {}) {
  ...
  for (const { instance, state } of instances) {
    ...
    try {
      ...
      const response = await fetchWithTimeout(url, NITTER_FETCH_OPTIONS, timeoutMs);
      ...
      $('.timeline-item').each((_, element) => {
        ...
        const views = snippetStats.views || 0;
        const bookmarks = snippetStats.bookmarks || 0;
...
export async function fetchTweetDetailsFromFallbackSources(tweetId, options = {}) {
  ...
  const jinaStatusFn = options.jinaStatusFn || fetchJinaStatusTweetById;
  const clients = {
    syndication: syndicationFn,
    oembed: oembedFn,
    jina_status: jinaStatusFn,
  };
