  ...
  for (const variant of variants) {
    ...
    if (nitterResults.length > 0) {
      ...
      if (ddgResults.length > 0) {
        ...
        let bingResults = [];
        variantInfo.sourceAttempts.bing = (variantInfo.sourceAttempts.bing || 0) + 1;
        meta.sources.bing.attempts += 1;

        try {
          const bingResponse = await searchBingFn(variant.query, {
            timeoutMs: Math.min(SOURCE_TIMEOUT_MS, remaining)
          });
          bingResults = bingResponse.results || [];
          hadSuccessfulSourceResponse = true;
          if (!firstSuccessInstance && bingResponse.instance) {
            firstSuccessInstance = bingResponse.instance;
          }
        } catch {
          meta.sources.bing.failures += 1;
        }
        ...
        if (bingResults.length > 0) {
          ...
          let jinaResults = [];
          variantInfo.sourceAttempts.jina += 1;
          meta.sources.jina.attempts += 1;

          try {
            const jinaResponse = await searchJinaFn(variant.query, {
              timeoutMs: Math.min(SOURCE_TIMEOUT_MS, remaining)
            });
            jinaResults = jinaResponse.results || [];
            hadSuccessfulSourceResponse = true;
            if (!firstSuccessInstance && jinaResponse.instance) {
              firstSuccessInstance = jinaResponse.instance;
            }
          } catch {
            meta.sources.jina.failures += 1;
          }
