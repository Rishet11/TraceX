...
const SOURCE_QUERY_CACHE = new Map();
...
function readCachedSearch(key) {
  ...
  return cloneForCache(hit.body);
}

function writeCachedSearch(key, body) {
  SEARCH_CACHE.set(key, {
    expiresAt: Date.now() + CACHE_TTL_MS,
    body: cloneForCache(body),
  });
...
function getSourceCacheKey(source, query) {
  return `${String(source || '').toLowerCase()}::${String(query || '').trim()}`;
}

function readCachedSourceResult(source, query) {
  const key = getSourceCacheKey(source, query);
  const hit = SOURCE_QUERY_CACHE.get(key);
  if (!hit) return null;
  if (hit.expiresAt <= Date.now()) {
    SOURCE_QUERY_CACHE.delete(key);
    return null;
  }
  return cloneForCache(hit.body);
}

function writeCachedSourceResult(source, query, body) {
  const key = getSourceCacheKey(source, query);
  SOURCE_QUERY_CACHE.set(key, {
    expiresAt: Date.now() + SOURCE_CACHE_TTL_MS,
    body: cloneForCache(body),
  });
  if (SOURCE_QUERY_CACHE.size > SOURCE_CACHE_MAX_ENTRIES) {
    const oldestKey = SOURCE_QUERY_CACHE.keys().next().value;
    if (oldestKey) SOURCE_QUERY_CACHE.delete(oldestKey);
  }
}
...
async function runSourceSearch({
  ...
  if (enableSourceCache) {
    const cached = readCachedSourceResult(source, query);
  ...
  if (enableSourceCache) {
    writeCachedSourceResult(source, query, payload);
  }
...
export async function runSearchPipeline(
  ...
  if (enableCache) {
    const cachedBody = readCachedSearch(cacheKey);
    if (cachedBody) {
      cachedBody.meta = {
        ...(cachedBody.meta || {}),
        cacheHit: true,
  ...
  if (enrichedResults.length > 0) {
    ...
    if (enableCache) writeCachedSearch(cacheKey, body);
    return {
      status: 200,
      body
    };
